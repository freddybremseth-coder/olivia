...
const SENSOR_RANGES: Record<string, SensorRange> = {
  soil_moisture: {
    ...
    optMin: 35,
    optMax: 65,
    description: 'Viktigste styringsverdi for vanning. Må tolkes etter jordtype, dybde og alder på trærne.',
  },
  soil_temperature: {
    ...
    optMin: 16,
    optMax: 30,
    description: 'Påvirker rotaktivitet, næringsopptak og vannrespons.',
  },
  soil_ec: {
    label: 'Jord EC / salt',
    unit: 'dS/m',
    ...
    optMin: 0.4,
    optMax: 2.0,
    description: 'Brukes til å følge saltoppbygging i jorden, spesielt ved dryppvanning.',
  },
  soil_ph: {
    label: 'Jord pH',
    unit: 'pH',
    ...
    optMin: 6,
    optMax: 7.8,
    description: 'Oliven tåler en del variasjon, men pH påvirker næringstilgang og jordhelse.',
  },
  water_ec: {
    label: 'Vann EC / salt',
    unit: 'dS/m',
    ...
    optMin: 0.2,
    optMax: 1.5,
    description: 'Kontrollerer saltnivå i vanningsvannet før det bygger seg opp i jorden.',
  },
  water_ph: {
    label: 'Vann pH',
    unit: 'pH',
    ...
    optMin: 6,
    optMax: 7.8,
    description: 'Viktig for vannkvalitet, gjødsling og langsiktig jordbalanse.',
  },
  flow: {
    label: 'Flow',
    unit: 'L/min',
    ...
    optMin: 10,
    optMax: 120,
    description: 'Viser om vann faktisk går ut i vanningssektoren.',
  },
  pressure: {
    label: 'Trykk',
    unit: 'bar',
    ...
    optMin: 1,
    optMax: 3.5,
    description: 'Avslører lekkasjer, tette dryppslanger eller pumpeproblemer.',
  },
  rain: {
    label: 'Regn',
    unit: 'mm',
    ...
    optMin: 0,
    optMax: 25,
    description: 'Brukes sammen med jordfuktighet for å justere vanning.',
  },
  air_temperature: {
    ...
    optMin: 12,
    optMax: 34,
    description: 'Viktig for tørkestress, frostfare, blomstring og modning.',
  },
  air_humidity: {
    label: 'Luftfuktighet',
    unit: '%',
    ...
    optMin: 35,
    optMax: 75,
    description: 'Brukes med temperatur og bladfukt for sykdomsrisiko.',
  },
  leaf_wetness: {
    label: 'Bladfukt',
    unit: '%',
    ...
    optMin: 0,
    optMax: 40,
    description: 'Høy bladfukt over tid kan indikere økt sykdomsrisiko.',
  },
  battery: {
    label: 'Batteri',
    unit: '%',
    ...
    optMin: 30,
    optMax: 100,
    description: 'Brukes til drift og vedlikehold av sensorer.',
  },
};
...
function generateHistory(baseValue: number, count = 24): SensorReadingPoint[] {
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      value: Math.round((safeBase + noise) * 10) / 10,
    };
  });
}
...
const DEFAULT_SENSORS: ExtendedSensor[] = [
  {
    ...
    signal_rssi: -76,
    source: 'simulation',
    calibrated_at: '2026-05-01T08:00:00Z',
    measured_at: new Date().toISOString(),
    lastUpdated: '2 min siden',
  },
  {
    id: 's2',
    sensor_id: 'DA-BIAR-SOIL-T-30-A',
    name: 'Sone A - Jordtemp 30 cm',
  ...
  {
    id: 's4',
    sensor_id: 'DA-BIAR-WATER-EC',
    name: 'Vanningsvann - EC',
    type: 'water_ec',
    value: '1.2',
    unit: 'dS/m',
    parcelId: '',
...
const IoTDashboard: React.FC = () => {
  ...
  const loadData = () => {
    ...
    loadedSensors = loadedSensors.map(migrateLegacySensor).map(sensor => ({
      ...sensor,
      history: sensor.history || generateHistory(parseFloat(sensor.value)),
    }));
  ...
  const simulateUpdate = () => {
    setSensors(prev => {
      const updated = prev.map(sensor => {
        ...
        const newReading: SensorReadingPoint = {
        ...
        return {
          ...
          history: [...(sensor.history || []).slice(-23), newReading],
        };
      });
  ...
  const handleAddSensor = () => {
    ...
    const range = SENSOR_RANGES[normalizedType];
    const defaultVal = defaultValueForType(normalizedType);
    const sensor: ExtendedSensor = {
      id: 'S' + Date.now(),
      sensor_id: newSensor.sensor_id || 'DA-BIAR-' + Date.now(),
      name: newSensor.name || 'Ny sensor',
      type: normalizedType,
      value: newSensor.value || defaultVal,
      unit: range.unit,
      parcelId: newSensor.parcelId || newSensor.parcel_id || '',
      parcel_id: newSensor.parcel_id || newSensor.parcelId || '',
      ...
      status: 'Online',
      lastUpdated: 'Akkurat nå',
      history: generateHistory(parseFloat(newSensor.value || defaultVal)),
    };
  ...
  const getDecisionSummary = () => {
    const dry = sensors.find(sensor => normalizeSensorType(sensor.type) === 'soil_moisture' && getSensorStatus(sensor) !== 'optimal');
    const salt = sensors.find(sensor => ['soil_ec', 'water_ec'].includes(normalizeSensorType(sensor.type)) && getSensorStatus(sensor) !== 'optimal');
    const pressure = sensors.find(sensor => normalizeSensorType(sensor.type) === 'pressure' && getSensorStatus(sensor) !== 'optimal');
    if (dry) return `Prioriter vanning/kontroll i ${dry.zone_id || dry.name}. Jordfukt er ${dry.value}${dry.unit}.`;
    if (salt) return `Følg med på saltnivå i ${salt.name}. Måling: ${salt.value}${salt.unit}.`;
    if (pressure) return `Sjekk vanningssektor: ${pressure.name} viser ${pressure.value}${pressure.unit}.`;
    return 'Ingen kritiske avvik i demo-data akkurat nå. Fortsett overvåkning og sjekk feltobservasjoner.';
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm font-bold uppercase tracking-widest mt-1">
            DonaAnna · Biar {DONA_ANNA_BIAR_SEASON_SETTINGS.altitude_m} moh. · {sensors.length} sensorer · Oppdatert {lastRefresh.toLocaleTimeString('no-NO', { hour: '2-digit', minute: '2-digit' })}
          </p>
        </div>
      ...
      <div className="glass rounded-[2rem] p-6 border border-green-500/20 bg-green-500/5">
        ...
        <p className="text-white font-bold">{getDecisionSummary()}</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={item.label} className={`glass rounded-[2rem] p-6 border ${item.bg}`}>
            <div className={`${item.color} mb-2`}>{item.icon}</div>
            <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">{item.label}</p>
            <p className={`text-4xl font-bold ${item.color} mt-1`}>{item.value}</p>
          </div>
        ))}
      </div>
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      <div className="p-2.5 rounded-xl" style={{ background: color + '18', color }}>
                        {SENSOR_ICONS[normalizedType] || <Activity size={20} />}
                      </div>
                      <div>
                        <p className="text-sm font-bold text-white">{sensor.name}</p>
                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">
                          {range?.label || normalizedType} {sensor.depth_cm ? `· ${sensor.depth_cm} cm` : ''} {parcel ? `· ${parcel.name}` : ''}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="flex justify-between items-center mt-2 pt-2 border-t border-white/5">
                    ...
                    <button
                      onClick={event => { event.stopPropagation(); handleDeleteSensor(sensor.id); }}
        ...
        <div className="lg:col-span-5 space-y-6">
              ...
              <div className="glass rounded-[2.5rem] p-8 border border-white/10 space-y-6 animate-in slide-in-from-right-4">
                  ...
                  <div className="h-48">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={selectedSensor.history}>
                        ...
                        {range && (
                          <>
                            <ReferenceLine y={range.optMin} stroke="#22c55e" strokeDasharray="4 4" strokeWidth={1} />
                            <ReferenceLine y={range.optMax} stroke="#22c55e" strokeDasharray="4 4" strokeWidth={1} />
                          </>
                        )}
                        <Area type="monotone" dataKey="value" stroke={color} fill={`url(#grad-${selectedSensor.id})`} strokeWidth={2} name={selectedSensor.unit} />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    { label: 'Type', value: range?.label || normalizedType },
                ...
                {range && (
                  <div className="p-5 bg-green-500/5 rounded-2xl border border-green-500/20">
                    <p className="text-[10px] font-bold text-green-400 uppercase tracking-widest mb-3">Optimalt område – DonaAnna/Biar</p>
                    <p className="text-xs text-slate-400 leading-relaxed mb-4">{range.description}</p>
                    <div className="grid grid-cols-3 gap-3 text-center">
                      <div><p className="text-[9px] text-slate-500">Min</p><p className="text-sm font-bold text-white">{range.min}</p></div>
                      <div className="border-x border-white/10"><p className="text-[9px] text-green-400">Optimalt</p><p className="text-sm font-bold text-green-400">{range.optMin}–{range.optMax}</p></div>
                      <div><p className="text-[9px] text-slate-500">Maks</p><p className="text-sm font-bold text-white">{range.max}</p></div>
                    </div>
                  </div>
                )}
              </div>
          ...
          {(criticalSensors > 0 || warningSensors > 0) && (
            <div className="glass rounded-[2.5rem] p-6 border border-red-500/20 bg-red-500/5 space-y-3">
              ...
              {sensors
                .filter(sensor => getSensorStatus(sensor) !== 'optimal' || sensor.status === 'Offline')
                .map(sensor => {
                  const st = getSensorStatus(sensor);
                  const range = SENSOR_RANGES[normalizeSensorType(sensor.type)];
                  return (
                    <div key={sensor.id} className={`flex items-center justify-between p-3 rounded-xl border ${st === 'critical' ? 'bg-red-500/10 border-red-500/20' : 'bg-yellow-500/10 border-yellow-500/20'}`}>
                      <div>
                        ...
                        <p className={`text-[9px] font-bold ${st === 'critical' ? 'text-red-400' : 'text-yellow-400'}`}>
                          {sensor.status === 'Offline' ? 'Ingen tilkobling' : `${sensor.value}${sensor.unit} – ${range?.label || sensor.type} utenfor ønsket område`}
                        </p>
                      </div>
          ...
          <div className="glass w-full max-w-md rounded-[2.5rem] p-10 border border-white/20 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input className="w-full bg-black/40 border border-white/10 rounded-2xl px-6 py-4 text-white focus:outline-none" placeholder="Eks: Sone A - Jordfukt 30 cm" value={newSensor.name || ''} onChange={event => setNewSensor({ ...newSensor, name: event.target.value })} />
              </div>

              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Sensor-ID</label>
                <input className="w-full bg-black/40 border border-white/10 rounded-2xl px-6 py-4 text-white focus:outline-none" placeholder="Eks: DA-BIAR-SOIL-M-30-A" value={newSensor.sensor_id || ''} onChange={event => setNewSensor({ ...newSensor, sensor_id: event.target.value })} />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={event => {
                    const type = event.target.value as SensorType;
                    const range = SENSOR_RANGES[normalizeSensorType(type)];
                    setNewSensor({ ...newSensor, type, unit: range.unit, value: defaultValueForType(type) });
                  }}
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Sone</label>
                  <input className="w-full bg-black/40 border border-white/10 rounded-2xl px-4 py-3 text-white focus:outline-none" placeholder="zone-a" value={newSensor.zone_id || ''} onChange={event => setNewSensor({ ...newSensor, zone_id: event.target.value })} />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Dybde cm</label>
                  <input type="number" className="w-full bg-black/40 border border-white/10 rounded-2xl px-4 py-3 text-white focus:outline-none" placeholder="30" value={newSensor.depth_cm || ''} onChange={event => setNewSensor({ ...newSensor, depth_cm: event.target.value ? Number(event.target.value) : undefined })} />
                </div>
              </div>

              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Tregruppe</label>
                <input className="w-full bg-black/40 border border-white/10 rounded-2xl px-6 py-4 text-white focus:outline-none" placeholder="Eks: Unge Gordal" value={newSensor.tree_group || ''} onChange={event => setNewSensor({ ...newSensor, tree_group: event.target.value })} />
              </div>

              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Parsell</label>
                <select className="w-full bg-black/40 border border-white/10 rounded-2xl px-6 py-4 text-white focus:outline-none" value={newSensor.parcelId || ''} onChange={event => setNewSensor({ ...newSensor, parcelId: event.target.value, parcel_id: event.target.value })}>
