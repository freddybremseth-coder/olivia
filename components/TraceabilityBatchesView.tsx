      ...
      <div className="glass rounded-[2rem] p-6 border border-green-500/20 bg-green-500/5">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-white font-bold">Hver batch får egen batchkode og QR-slug.</p>
            <p className="text-xs text-slate-500 mt-2">Neste steg kan være offentlig QR-side for hvert produkt: sort, høstedato, 650 moh., labverdier, bilder fra feltet, prosessering og historien om DonaAnna.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        {batches.map(batch => (
          <div key={batch.id} className={`glass rounded-[2rem] p-6 border ${statusClass(batch.status)}`}>
            ...
            <div className="mt-5 pt-4 border-t border-white/10 flex items-center gap-2 text-xs text-slate-500"><Link2 size={14} /> Fremtidig QR-side: /trace/{batch.qr_slug}</div>
          </div>
        ))}
      </div>
