...
export default async function handler(req: IncomingMessage & { method?: string; body?: unknown }, res: ServerResponse) {
  ...
  if (!apiKey) {
    sendJson(req, res, 503, { error: { message: 'Claude er ikke konfigurert: legg inn ANTHROPIC_API_KEY i Vercel Environment Variables.' } });
    return;
  }
  ...
  try {
    ...
    if (!body || body === '{}') {
      sendJson(req, res, 400, { error: { message: 'Tom request body til Anthropic proxy.' } });
      return;
    }
    ...
    sendJson(req, res, 500, { error: { message: error?.message || 'Anthropic proxy failed.' } });
  }
}
