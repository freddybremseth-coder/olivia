...
export default async function handler(req: IncomingMessage & { method?: string; body?: unknown }, res: ServerResponse) {
  ...
  if (!apiKey) {
    sendJson(res, 503, { error: { message: 'Gemini er ikke konfigurert: legg inn GEMINI_API_KEY eller GOOGLE_API_KEY i Vercel Environment Variables.' } });
    return;
  }
  ...
  try {
    ...
    if (!body || body === '{}') {
      sendJson(res, 400, { error: { message: 'Tom request body til Gemini vision proxy.' } });
      return;
    }
    ...
    sendJson(res, 500, { error: { message: error?.message || 'Gemini vision proxy failed.' } });
  }
}
